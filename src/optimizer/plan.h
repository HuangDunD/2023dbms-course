...
typedef enum PlanTag{
    ...
    T_IndexScan,
    T_NestLoop,
    T_Hash, // 新增hash join类别(话说不应该是只有一个join类别, 然后优化器根据条件再选择具体哪种join吗?)
    T_Sort,
    T_Projection
} PlanTag;
...
{
    public:
        SortPlan(PlanTag tag, std::shared_ptr<Plan> subplan, std::vector<ColMeta> sel_col, std::vector<bool> is_desc_arr)
        {
            Plan::tag = tag;
            subplan_ = std::move(subplan);
            sel_col_ = sel_col;
            is_desc_arr_ = is_desc_arr;
        }
        ~SortPlan(){}
        std::shared_ptr<Plan> subplan_;
        // ColMeta sel_col_;
        std::vector<ColMeta> sel_col_;
        std::vector<bool> is_desc_arr_;
};
...
class DMLPlan : public Plan
{
    public:
        DMLPlan(PlanTag tag, std::shared_ptr<Plan> subplan,std::string tab_name,
                ...
                std::vector<SetClause> set_clauses, std::string load_file_name = "" ) : subplan_(std::move(subplan)), tab_name_(std::move(tab_name)), values_(std::move(values)), conds_(std::move(conds)), set_clauses_(std::move(set_clauses)), load_file_name_(std::move(load_file_name))
        {
            Plan::tag = tag;
        }
        ~DMLPlan(){}
...
// help; show tables; desc tables; begin; abort; commit; rollback; set语句对应的plan
class OtherPlan : public Plan
{
    public:
        OtherPlan(PlanTag tag, std::string tab_name, std::string value = "")
        {
            Plan::tag = tag;
            tab_name_ = std::move(tab_name);            
