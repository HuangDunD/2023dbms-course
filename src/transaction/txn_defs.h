   ...
   public:
    ...
    inline int64_t Get() const {
        if (type_ == LockDataType::TABLE) {
            ...
            return ((static_cast<int64_t>(type_)) << 62) | ((static_cast<int64_t>(fd_)) << 31) |
...
/* 事务回滚原因 */
enum class AbortReason { LOCK_ON_SHIRINKING = 0, UPGRADE_CONFLICT, DEADLOCK_PREVENTION, COMMIT_ABOOTED_TRANSACTION, NESTLOOPJOIN_FILE_FAILURE };
   ...
   public:
    ...
    std::string GetInfo() {
        switch (abort_reason_) {
            ...
            case AbortReason::COMMIT_ABOOTED_TRANSACTION: {
                return "Transaction " + std::to_string(txn_id_) + " commit but it is aborted\n";
            } break;

            case AbortReason::NESTLOOPJOIN_FILE_FAILURE: {
                return "Transaction " + std::to_string(txn_id_) + " nestloop join failed\n";
            } break;
            default: {
