...
public:
    ...
    BeginLogRecord(txn_id_t txn_id, lsn_t prev_lsn) : BeginLogRecord() {
        log_tid_ = txn_id;
        prev_lsn_ = prev_lsn;
    }
    // 序列化Begin日志记录到dest中
...
public:
    ...
    RmRecord old_value_;        // 原来的的记录
    RmRecord new_value_;        // 原来的的记录
    Rid rid_;                   // 记录删除的位置
    char* table_name_;          // 插入记录的表名称
    size_t table_name_size_;    // 表名称的大小
};
...
public:
    ...
    bool is_full(int append_size) {
        if(offset_ + append_size + sizeof(int)*10 > LOG_BUFFER_SIZE)
            return true;
        return false;
    }
...
public:
    ...
    void RunFlushThread();
    void ForceFlush(lsn_t lsn);

    LogBuffer* get_log_buffer() { return log_buffer_; }
    LogBuffer* get_flush_buffer() { return flush_buffer_; }
    void set_persist_lsn(lsn_t persist_lsn) { persist_lsn_ = persist_lsn; }
    void set_global_lsn(lsn_t global_lsn) { global_lsn_ = global_lsn; }
private:    
    std::atomic<lsn_t> global_lsn_{0};  // 全局lsn，递增，用于为每条记录分发lsn
    std::mutex latch_;                  // 用于对log_buffer_的互斥访问
    LogBuffer *log_buffer_;              // 日志写入缓冲区
    LogBuffer *flush_buffer_;              // 日志写入缓冲区
    ...
    std::condition_variable operation_cv_; // 条件变量, 用于可添加新日志线程的唤醒
