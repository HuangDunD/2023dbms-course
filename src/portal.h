   ...
   public:
        ...
        if (auto x = std::dynamic_pointer_cast<OtherPlan>(plan)) {
            ...
            switch(x->tag) {
                case T_select:
                {
                    ...
                    if (p->sel_cols_.at(0).agg_type == ast::SV_AGG_NONE) {
                        return std::make_shared<PortalStmt>(PORTAL_ONE_SELECT, std::move(p->sel_cols_), std::move(root), plan);
                    } else {
                        return std::make_shared<PortalStmt>(PORTAL_AGG_SELECT, std::move(p->sel_cols_), std::move(root), plan);                        
                    }
                }
                    
                case T_Update:
                {
                    // 直接上X锁，这里不使用IX，因为可能会造成不可重复读/幻读
                    context->lock_mgr_->lock_exclusive_on_table(context->txn_, sm_manager_->fhs_.at(x->tab_name_)->GetFd());

                    std::unique_ptr<AbstractExecutor> scan= convert_plan_executor(x->subplan_, context);

                    // // 这里进行加锁粒度的优化
                    // IndexScanExecutor* index_scan = dynamic_cast<IndexScanExecutor*>(scan.get());
                ...
                {   
                    ...
                    std::unique_ptr<AbstractExecutor> root =
                            std::make_unique<InsertExecutor>(sm_manager_, x->tab_name_, std::move(x->values_), x->load_file_name_, context);
    ...
    void run(std::shared_ptr<PortalStmt> portal, QlManager* ql, txn_id_t *txn_id, Context *context){
        switch(portal->tag) {
            ...
            {
                ql->select_from_with_aggregate(std::move(portal->root), std::move(portal->sel_cols), context);
                break;        
            }
    ...
    std::unique_ptr<AbstractExecutor> convert_plan_executor(std::shared_ptr<Plan> plan, Context *context)
    {
        if(auto x = std::dynamic_pointer_cast<ProjectionPlan>(plan)){
            ...
            // auto temp = convert_plan_executor(x->subplan_, context);
            ...
            {
            case T_NestLoop:{
                std::unique_ptr<AbstractExecutor> join = std::make_unique<NestedLoopJoinExecutor>(std::move(left), std::move(right), std::move(x->conds_), context);
                return join;
            }
            case T_Hash:{
                std::unique_ptr<AbstractExecutor> join = std::make_unique<HashJoinExecutor>(std::move(left), std::move(right), std::move(x->conds_));
                return join;
            }
            default:
                assert(0);  // 只有出错才会运行到这里
                break;
            }
            
        } else if(auto x = std::dynamic_pointer_cast<SortPlan>(plan)) {
            return std::make_unique<SortExecutor>(convert_plan_executor(x->subplan_, context), 
                                            x->sel_col_, x->is_desc_arr_);
        }
        return nullptr;
    }
