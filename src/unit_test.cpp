...
auto buffer_pool_manager = std::make_unique<BufferPoolManager>(TEST_BUFFER_POOL_SIZE, disk_manager.get(), nullptr);
...
TEST_F(BufferPoolManagerTest, SampleTest) {
    ...
    auto bpm = std::make_unique<BufferPoolManager>(buffer_pool_size, disk_manager, nullptr);
    // create tmp PageId
...
//         std::shared_ptr<BufferPoolManager> bpm{new BufferPoolManager(50, disk_manager, nullptr)};
...
//                     EXPECT_NE(nullptr, page);
//                     ASSERT_NE(nullptr, page);
//                     strcpy(page->get_data(), std::to_string(page_ids[j].page_no+10).c_str());  // NOLINT
//                     EXPECT_EQ(1, bpm->unpin_page(page_ids[j], true));
//                 }
//                 for (int j = 0; j < 10; j++) {
//                     auto page = bpm->fetch_page(page_ids[j]);
//                     EXPECT_NE(nullptr, page);
//                     ASSERT_NE(nullptr, page);
//                     if(std::strcmp(std::to_string(page_ids[j].page_no+10).c_str(), (page->get_data()))){
//                         std::cout << std::to_string(page_ids[j].page_no+10).c_str() << " " << page->get_data() << std::endl;
//                     }
//                     EXPECT_EQ(0, std::strcmp(std::to_string(page_ids[j].page_no+10).c_str(), (page->get_data())));
//                     EXPECT_EQ(1, bpm->unpin_page(page_ids[j], true));
//                 }
//                 for (int j = 0; j < 10; j++) {
...
// TEST_F(BufferPoolManagerConcurrencyTest, ConcurrencyTest2) {
//     const int num_threads = 5;
...
//         // create BufferPoolManager
//         auto disk_manager = BufferPoolManagerConcurrencyTest::disk_manager_.get();
//         std::shared_ptr<BufferPoolManager> bpm{new BufferPoolManager(10, disk_manager, nullptr)};
...
class BufferPoolManagerPerformanceTest : public ::testing::Test {
   public:
    std::unique_ptr<DiskManager> disk_manager_;
    // int fd_ = -1;  // 此文件描述符为disk_manager_->open_file的返回值
    int fd_[20];
   ...
   public:
    ...
    void SetUp() override {
        ...
        // 如果测试文件存在，则先删除原文件（最后留下来的文件存的是最后一个测试点的数据）
        for(int i=0; i< 20; i++){
            if (disk_manager_->is_file(TEST_FILE_NAME_CCUR + std::to_string(i) )) {
                disk_manager_->destroy_file(TEST_FILE_NAME_CCUR + std::to_string(i));
            }
            // 创建测试文件
            disk_manager_->create_file(TEST_FILE_NAME_CCUR + std::to_string(i));
            assert(disk_manager_->is_file(TEST_FILE_NAME_CCUR + std::to_string(i)));
            // 打开测试文件
            fd_[i] = disk_manager_->open_file(TEST_FILE_NAME_CCUR + std::to_string(i));
            assert(fd_[i] != -1);
        }
    ...
    void TearDown() override {
        for(int i=0; i< 20; i++){
            disk_manager_->close_file(fd_[i]);
        }
        // disk_manager_->destroy_file(TEST_FILE_NAME_CCUR);  // you can choose to delete the file
...
TEST_F(BufferPoolManagerPerformanceTest, performance) {
    ...
    int fd[20];
    for(int i=0; i<20; i++){
        fd[i] = BufferPoolManagerPerformanceTest::fd_[i];
    }

    for (int run = 0; run < num_runs; run++) {
        // create BufferPoolManager
        auto disk_manager = BufferPoolManagerPerformanceTest::disk_manager_.get();
        std::shared_ptr<BufferPoolManager> bpm{new BufferPoolManager(100000, disk_manager, nullptr)};
        
        auto start = std::chrono::high_resolution_clock::now();

        for(int fd_i=0; fd_i<20; fd_i++){
            for (int i = 0; i < 20000; i++) {
                PageId temp_page_id = {.fd = fd[fd_i], .page_no = INVALID_PAGE_ID};
                auto new_page = bpm->new_page(&temp_page_id);
                EXPECT_NE(nullptr, new_page);
                ASSERT_NE(nullptr, new_page);
                strcpy(new_page->get_data(), std::to_string(temp_page_id.page_no).c_str());  // NOLINT
            ...
            std::cout << "Load " << fd[fd_i] << " finish." << std::endl;
        }

        std::vector<std::thread> threads;
        for (int tid = 0; tid < num_threads; tid++) {
            threads.push_back(std::thread([&bpm, tid, &fd]() {  // NOLINT
                std::random_device rd;  // 获取真随机数生成器的种子
                std::mt19937 gen(rd()); // 使用 Mersenne Twister 作为随机数引擎
                std::uniform_int_distribution<int> dist_fd(0, 19);
                std::uniform_int_distribution<int> dist_page(0, 19999);
                std::uniform_int_distribution<int> dist_rw(0, 19999);

                for (int j = 1; j <= 200000; j++) {
                    PageId pageid{fd[dist_fd(gen)], dist_page(gen)};
                    auto page = bpm->fetch_page(pageid);
                    EXPECT_NE(nullptr, page);
                    ASSERT_NE(nullptr, page);
                    EXPECT_EQ(1, bpm->unpin_page(pageid, dist_rw(gen)));

                    if(j % 10000 == 0){
                        std::cout << "tid: " << tid << " fetch: " << j << std::endl;
                    }
                }
            }));
        }  // end loop tid=[0,num_threads)
        ...
        // 输出运行时间
        std::cout << "run time: " << duration.count() << " 秒" << std::endl;
...
//     auto disk_manager = std::make_unique<DiskManager>();
//     auto buffer_pool_manager = std::make_unique<BufferPoolManager>(BUFFER_POOL_SIZE, disk_manager.get(), nullptr);
